...
def calculate_similarities(current_game, games, genres, tags, game_details, tfidf_games):
    ...
    array_tfidf = np.matrix([tfidf_game.tfidf for tfidf_game in tfidf_games])

    smile = tfidf_current_game * array_tfidf / (np.linalg.norm(tfidf_current_game) * np.linalg.norm(array_tfidf))

    '''for tfidf_game in tfidf_games:
        tfidf = np.asarray(tfidf_game.tfidf)
        #cosinus_similarity = games_similarity[tfidf_game.steam_id] = np.dot(tfidf, tfidf_current_game) / (
                    #np.linalg.norm(tfidf) * (np.linalg.norm(tfidf_current_game)))
        cosinus_similarity=sk.metrics.pairwise.linear_kernel(tfidf, tfidf_current_game)
